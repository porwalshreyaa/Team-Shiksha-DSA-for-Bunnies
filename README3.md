# DSA For Bunnies - Part 3

## Session 21 - Hashing and Array(29 jan 26) (Pending)

### Discussed

1. [Longest Substring Without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/description/)   |   [Solution]()
    #### [Excalidraw](https://excalidraw.com/#json=uXotFeeLN_fVVUR01W4q5,TnDSl4uCv8LL8I5JNPnsFA)

### Assignment Problems:

1. [Jewels and Stones](https://leetcode.com/problems/jewels-and-stones/description/)   |   [Solution]()

## Session 22 - Hashing and Array(30 jan 26) (Pending)

### Discussed

1. [Longest Substring with at Most Two Distinct Characters](https://www.geeksforgeeks.org/dsa/longest-substring-with-at-most-two-distinct-characters/)   |   [Solution]()
    #### [Excalidraw](https://excalidraw.com/#json=4j7xKF-HYAVUeQNybYwWU,8UuL57S5MVovQNWLAmHi5w)

### Assignment Problems:

1. [Longest k Unique Characters Substring](https://www.geeksforgeeks.org/problems/longest-k-unique-characters-substring0853/1)   |   [Solution]()
2. [Distinct Characters](https://www.naukri.com/code360/problems/distinct-characters_2221410)   |   [Solution]()

## Session 23 - Array(2 Feb 26) (Brainstorming...)

### Discussed

1. [Longest Substring with at least k repeating characters](https://leetcode.com/problems/longest-substring-with-at-least-k-repeating-characters/)   |   [Solution](./solutions/Session-22-30-Jan/Longest_Substring_with_at_least_k.py)
    #### [Excalidraw](https://excalidraw.com/#json=foUNkgiuJeL25XBs4ydOf,kEWaBRnnTy6tk0jGwDphtg)

### Assignment Problems:

1. [Repeated DNA Sequences](https://leetcode.com/problems/repeated-dna-sequences/description/)   |   [Solution](./solutions/Session-23-2-Feb/Repeated_DNA_Sequences.py)

## Session 24 - Binary Search(3 Feb 26)

### Discussed

1. [Binary Search](https://leetcode.com/problems/binary-search/description/)   |   [Solution](./solutions/Session-24-3-Feb/binary_search.py)
    #### [Excalidraw](https://excalidraw.com/#json=_PsKnnwn8DSHCipvM3wuQ,aHgotYprv5iUTbyT7VGJpQ)

### Assignment Problems:

1. [Search Insert Position of k in a Sorted Array](https://www.geeksforgeeks.org/problems/search-insert-position-of-k-in-a-sorted-array/1)   |   [Solution](./solutions/Session-24-3-Feb/search_insert_position_of_k_in_sorted_array.py)

## Session 25 - Binary Search(4 Feb 26)

### Discussed

1. [Capacity to ship packages](https://leetcode.com/problems/capacity-to-ship-packages-within-d-days/)   -   [Youtube link for revision](https://www.youtube.com/watch?v=MG-Ac4TAvTY)   |   [Solution](./solutions/Session-25-4-Feb/capacity_to_ship.py)
    #### [Excalidraw](https://excalidraw.com/#json=vTo03307kcFlfi8YLlh5U,L7yAHbDPVSTPRzmiyfff9Q)

### Assignment Problems:

1. [Implement Lower Bound](https://www.geeksforgeeks.org/problems/implement-lower-bound/1)   |   [Solution](./solutions/Session-25-4-Feb/implement_lower_bound.py)

2. [Implement Upper Bound](https://www.geeksforgeeks.org/problems/implement-upper-bound/1)   |   [Solution](./solutions/Session-25-4-Feb/implement_upper_bound.py)

## Session 26 - Binary Search(5 Feb 26) (Pending)

### Discussed

1. [Cutting Ribbons](https://algo.monster/liteproblems/1891)

    #### [Excalidraw](https://excalidraw.com/#json=tVJDdLaF7UznYpeM07Gj1,n6Yg5mtffud8cgmzlMYdvw)

### Assignemnt Problems:

1. [Count Complete Subarrays](https://leetcode.com/problems/count-complete-subarrays-in-an-array/)


## Session 27 - Binary Search(6 Feb 26) (Pending)

### Discussed

1. [Search a 2D Matrix](https://leetcode.com/problems/search-a-2d-matrix/)
    #### [Excalidraw](https://excalidraw.com/#json=HHUn9XbeXqEIubdMdDLY1,zJVjVvuctomatTnxYDFr5Q)

### Assignment Problems:

1. [Find first and last position of element](https://leetcode.com/problems/find-first-and-last-position-of-element-in-sorted-array/description/)

2. [Aggressive Cows](https://www.geeksforgeeks.org/problems/aggressive-cows/1)

3. Variation on top of [Search a 2D Matrix](https://leetcode.com/problems/search-a-2d-matrix/)
    - Each column is sorted in non-decreasing order.
    - The first integer of each col is greater than the last integer of the previous col.